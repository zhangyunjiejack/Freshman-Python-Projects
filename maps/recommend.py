...
def find_closest(location, centroids):
    ...
    # BEGIN Question 3
    "*** REPLACE THIS LINE ***"
    shortest = min(distance(location, choice) for choice in centroids)
    for choice in centroids:
        if distance(location, choice) == shortest:
            return choice
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** REPLACE THIS LINE ***"
    lst = [[find_closest(restaurant_location(restaurant), centroids), restaurant] for restaurant in restaurants]
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        lat += [restaurant_location(restaurant)[0]]
        longitude += [restaurant_location(restaurant)[1]]
    return [mean(lat), mean(longitude)]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            all_rate[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            all_rate[restaurant_name(restaurant)] = predictor(restaurant)
