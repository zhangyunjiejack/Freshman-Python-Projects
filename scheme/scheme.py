...
def eval_all(expressions, env):
    ...
    if (expressions is not nil):
        if expressions.second is nil:
            ...
            scheme_eval(expressions.first, env, False)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        target_value = scheme_eval(expressions.second.first, env)
        env.define(target, target_value)
        return target
        # END Question 5A
    ...
    if expressions is nil:
        ...
        while expressions:
            if expressions.second:
                expr = scheme_eval(expressions.first, env, False)
                if scheme_false(expr):
                    return False
                else:
                    expressions = expressions.second
            else:
                expr = scheme_eval(expressions.first, env, True)
                expressions = expressions.second
        return expr
    # END Question 14B
...
def do_or_form(expressions, env):
    ...
    if expressions:
        while expressions:
            if expressions.second:
                exprr = scheme_eval(expressions.first, env, False)
                if scheme_true(exprr):
                    return exprr
                else:
                    expressions = expressions.second
            else:
...
def make_let_frame(bindings, env):
    ...
    args = bindings.map((lambda expr: expr.first))
    bindings.map((lambda expr: check_formals(args)))
    vals = bindings.map((lambda expr: scheme_eval(expr.second.first, env)))
    return env.make_child_frame(args, vals)
...
def check_formals(formals):
    ...
    while formals is not nil:
        ...
        if not scheme_symbolp(symbol):
            raise SchemeError("Non-symbol: {}".format(symbol))
        if symbol in used:
